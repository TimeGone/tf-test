...
layer1 = tf.nn.softsign(tf.matmul(X, W1) + b1)
...
RSD = tf.reduce_mean(tf.div(tf.abs(tf.subtract(hypothesis, Y)), Y))
accuracy = tf.reduce_mean(tf.cast(tf.div(tf.abs(tf.subtract(hypothesis, Y)), Y) < 0.001, dtype=tf.float32))
...
#        x_data = np.concatenate((x_data, np.random.rand(256, 2)), axis=0)
        ...
        if step % 250 == 0:
            #print(sess.run([hypothesis, tf.nn.relu(hypothesis)], feed_dict={X:x_test, Y:y_test}))
            print(step, sess.run(cost, feed_dict={X:x_data, Y:y_data}), sess.run(accuracy, feed_dict={X:x_test, Y:y_test}), sess.run(RSD, feed_dict={X:x_test, Y:y_test}))
