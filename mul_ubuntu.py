...
x_data = np.random.rand(65536, 2) * 0.1
y_data = x_data[:,:1] * x_data[:, 1:2]

x_test = np.random.rand(8192, 2) * 0.1
...
#        x_data = np.concatenate((x_data, np.random.rand(256, 2)), axis=0)
        ...
        if step % 100 == 0:
            print(step, sess.run([loss, accuracy], feed_dict={X:x_data, Y:y_data}), sess.run(relative_bias_mean, feed_dict={X:x_test, Y:y_test}), x_data.shape[0])
            if step >= 5000:
